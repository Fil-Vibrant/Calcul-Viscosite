...
GraphesViscosite::GraphesViscosite(QObject *parent) : QObject(parent)
{
    xChart = new QChart;
...
QChart* GraphesViscosite::drawXGraph(vector<double> xExp, vector<double> xCal, vector<double> frequencies)
{
    ...
    x->setRange(min-0.0000005, max+0.0000005);
    x->setTickCount(5);
    xf->setTickCount(20);
...
QChart* GraphesViscosite::drawYGraph(vector<double> yExp, vector<double> yCal, vector<double> frequencies)
{
    double min = yExp[0];
    double max = yExp[0];
    ...
    y->setRange(min-0.0000005, max+0.0000005);
    y->setTickCount(5);
    yf->setTickCount(20);
    QPen minorPen(QColor::fromRgb(76, 85, 92), 1, Qt::SolidLine);
